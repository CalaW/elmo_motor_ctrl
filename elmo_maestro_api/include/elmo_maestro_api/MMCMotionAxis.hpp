...
public:
    ...
    */
    void Dwell(ELMO_ULINT32 ulDwellTimeMs) ;

    /*! \fn unsigned int InsertNotificationFb()
    * \brief This function Insert a notification FB that will generate event when called
    * \return   return - 0 on success, otherwise throws CMMCException.
    */
    void InsertNotificationFb(ELMO_INT32 iEventCode) ;
    void InsertNotificationFb(MMC_INSNOTIFICATIONFB_IN stInParams) ;
    ...
    */
    void WaitUntilConditionFB(MMC_WAITUNTILCONDITIONFB_IN stInput) ;

    void WaitUntilConditionFBEX(MMC_WAITUNTILCONDITIONFBEX_IN stInput) ;
    ...
    */
    ...
    MC_PATH_REF CamTableInit(ELMO_ULINT32 ulMaxPoints,
                                    ...
                                    CURVE_TYPE_ENUM eCurveType=ePolynom5Interp) ;
    ...
    ELMO_INT32 CamTableAdd(MC_PATH_REF hMemHandle, ELMO_PDOUBLE dbTable, ELMO_UINT16 usColumns, ELMO_ULINT32 ulNumberOfPoints) ;

    ELMO_INT32 CamTableAddEx(MC_PATH_REF hMemHandle, ELMO_PDOUBLE dbTable, ELMO_UINT16 usColumns, ELMO_ULINT32 ulNumberOfPoints);

    ELMO_INT32 CamTableSet(MC_PATH_REF hMemHandle, ELMO_PDOUBLE dbTable, ELMO_UINT16 usColumns, ELMO_ULINT32 ulStartIndex, ELMO_ULINT32 ulNumberOfPoints) ;
//#ifdef _DEBUG
//    ELMO_INT32 CamTablePrint(MC_PATH_REF hMemHandle, ELMO_ULINT32 ulStartIndex, ELMO_ULINT32 ulNumberOfPoints) ;
//#endif  
    /**
    ...
    */
    int CamGetStatus(ELMO_ULINT32& ulEndOfProfile,
                     ...
                     ELMO_ULINT32& ulCycle,
                     ELMO_UINT16 & usStatus,
                     ELMO_INT16  & sErrorID, ELMO_UINT32& uiCamTableID) ;

    int CamSetProperty(ECAM_PROPERTIES_ENUM eProperty, ECAM_PERIODIC_ENUM ePeriodicMode) ;
     ...
     * \return void
     */
    virtual void MovePVT(MC_PATH_REF hMemHandle, MC_COORD_SYSTEM_ENUM eCoordSystem) ;
     ...
     */
    virtual ELMO_INT32 MovePT(MC_PATH_REF hMemHandle, MC_COORD_SYSTEM_ENUM eCoordSystem) ;

    /*! \fn void UnloadPVTTable()
     * \brief This function unloads PVT table
     * \return void
     */
    void UnloadPVTTable(MC_PATH_REF hMemHandle) ;
    void CamTableUnload(MC_PATH_REF hMemHandle)  { UnloadPVTTable(hMemHandle);} 
    void UnloadCamTable(MC_PATH_REF hMemHandle)  { UnloadPVTTable(hMemHandle);}  //same as CamTableUnload but compatible to PT/PVT naming convension.
    void UnloadPTTable (MC_PATH_REF hMemHandle)  {UnloadPVTTable(hMemHandle);}
    ...
    ELMO_UINT32 GetPVTTableIndex(MC_PATH_REF hMemHandle)  ;
    ELMO_UINT32 GetPTTableIndex(MC_PATH_REF hMemHandle)  
    {
    ...
    */
    ELMO_INT32 GetTableList(NC_MOTION_TABLE_TYPE_ENUM eTableType, ELMO_UINT32 (&uiHandlers)[MMC_MAX_JOURNAL_ENTRIES], ELMO_INT32 & iNum) ;

    /**
    * \fn int GetTableInfo(unsigned int uiHandler, char (&name)[MMC_TABLE_FILE_LENGTH+1]) ;
    ...
    * \return   return - 0 if success otherwise error or thrown CMMCException.
    */
    ELMO_INT32 GetTableInfo(ELMO_UINT32 uiHandler, ELMO_INT8 (&name)[MMC_TABLE_FILE_LENGTH+1]) ;
    ...
    /**
    * \fn int KillMotion(MC_PATH_REF hMemHandle)  ;
    * \brief this method stop profiler moiton on the spot.
    * \return   return - 0 if success otherwise error or thrown CMMCException.
    */
    ELMO_INT32 KillMotion()  ;
